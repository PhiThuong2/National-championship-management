...
class Command(BaseCommand):
    help = 'Tự động tạo dữ liệu kết quả trận đấu Vòng 21-25 (logic theo vị trí, CAHN toàn thắng)'

    def handle(self, *args, **kwargs):
        self.stdout.write("Bắt đầu tạo dữ liệu kết quả trận đấu Vòng 21-25...")
        ...
        # Tìm đội CAHN
        cahn = Team.objects.filter(name__icontains="Công An Hà Nội").first()
        if cahn:
            self.stdout.write(self.style.SUCCESS(f"Đã tìm thấy CLB ưu tiên: {cahn.name}"))
        else:
            self.stdout.write(self.style.WARNING("Không tìm thấy CLB 'Công An Hà Nội', sẽ chạy logic ngẫu nhiên."))

        for round_num in range(21, 26):
            ...
            if not matches.exists():
                ...
                for i in range(0, len(teams), 2):
                    if i + 1 < len(teams):
                        match_date = timezone.now() - timezone.timedelta(days=(26-round_num)) # Very recent past
            ...
            for match in matches:
                ...
                # Check priority for CAHN
                is_cahn_home = (cahn and match.home_team.id == cahn.id)
                is_cahn_away = (cahn and match.away_team.id == cahn.id)
                
                if is_cahn_home:
                    # CAHN home -> must win
                    away_goals = random.randint(0, 1)
                    home_goals = random.randint(away_goals + 1, away_goals + 3) # ensure win
                elif is_cahn_away:
                    # CAHN away -> must win
                    home_goals = random.randint(0, 1)
                    away_goals = random.randint(home_goals + 1, home_goals + 2) # ensure win
                else:
                    # Random for others
